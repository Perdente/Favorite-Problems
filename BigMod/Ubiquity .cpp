...
bool as_second(const pair<ll,ll> &a, const pair<ll,ll> &b) {return ((a.first+a.second)< (b.first+b.second)); }//sort the vector pair in assending order according to second element
...
{
	cin>>n;
	ans=(binpow(10ll,n)-binpow(9ll,n)*2+binpow(8ll,n))%mod;
