...
int a,b,c,i,j,k,m,n,o,p,q,r,x,y,z,pos,res,res1,test,cnt,cnt1,cnt2,sum,sum1,sum2,ans,ma,ma1,ma2,mi,mi1,mi2,temp,temp1,temp2;
...
void SectumSempra()
{
	...
	lp(i,n){
		lp(j,n){
			if(vp[i].first<vp[j].first and vp[i].second<vp[j].second){
				pair<int,int> upper_left={vp[i].first,vp[j].second};
				pair<int,int> lower_right={vp[j].first,vp[i].second};
				cnt+=(binary_search(all(vp),upper_left) and binary_search(all(vp),lower_right));
			}
		}
	}
	cout<<cnt<<endl;
