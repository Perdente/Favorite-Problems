...
1.Here we take input in a vector.Reduced the vector elements to set.Sort the vector in decreasing order.
2.Create a vector u(v.size()) to store the values that are decreased due to -r.
3.Store the elements of u in a map mp.
4.Now iterate through the vector u and if i+1 is already visited that means the monster is already killed in the process .So we don't need to increase the cnt.
*/
