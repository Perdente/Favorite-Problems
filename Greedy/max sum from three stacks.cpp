...
void SectumSempra()
{
	...
	St s1,s2,s3;
	temp=0;
	lp(i,n)temp+=a[i],pr1[i]=temp,s1.insert(pr1[i]);
	temp=0;
	lp(i,m)temp+=b[i],pr2[i]=temp,s2.insert(pr2[i]);
	temp=0;
	lp(i,p)temp+=c[i],pr3[i]=temp,s3.insert(pr3[i]);
	s1.insert(0);
	s2.insert(0);
	s3.insert(0);
	ma=0;
	LP(i,s1){
		if(s2.count(i) and s3.count(i))ma=max(ma,i);
	}
	cout<<ma;
	lol
}
