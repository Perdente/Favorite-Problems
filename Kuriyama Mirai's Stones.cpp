...
1.Take 2 arrays one is sorted (non decreasing order) other is non sorted.
2.Make prefix sum for both the arrays.
3.Get the range of sum prefix_sum[left]-prefix_sum[right-1]
...
int main()
{
	...
	while(q--)
	{
		...
		if(t==1)  cout<<(l>1?sum[r-1]-sum[l-2]:sum[r-1])<<endl;
		else cout<<(l>1?sum1[r-1]-sum1[l-2]:sum1[r-1])<<endl;
	}
	return 0;
}
