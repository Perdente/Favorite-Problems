...
//bipartite coloring + combinatorics
...
void SectumSempra()
{
	...
	function<bool(int)>dfs=[&](int u)->bool{
		vis[u]=true;
		LP(v,adj[u]){
			if(col[u]==col[v])return false;// not bipartite
			if(!vis[v]){
				col[v]=col[u]^1;
				col[v]==1?cnt1++:cnt++;//count the number of 0's and 1's color
	...
	auto solve=[&]()->int{
		ans=1;
		lp1(i,1,n){
			cnt=0,cnt1=1;//first initialize the color arbitarily cnt1 is parent color
			col[i]=1;//parent vertex color
			if(!vis[i]){
				if(!dfs(i)){
					return 0;// if the graph is not bipartite ans is 0
				}
				ans=(( ans * (power2[cnt] + power2[cnt1])%MOD) )%MOD;
				//as 2^i count can increase the time so we pre initialize power2 array.
			}
		}
		return ans;
	};
